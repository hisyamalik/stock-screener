...
class MT5ForexRobot:
    def __init__(
        ...
        drawdown_period_hours: int = 24,
        # === Scalping options ===
        enable_scalping: bool = False,
        scalping_timeframe: int = mt5.TIMEFRAME_M5,
        max_spread_points: int = 30,              # e.g., 30 = 3.0 pips on 5-digit broker
    ...
    def analyze_market_scalping(self, symbol: str) -> str:
        ...
        Long:
            EMA9 > EMA21 AND RSI3 < 20 AND close crosses above lower band
        Short:
            EMA9 < EMA21 AND RSI3 > 80 AND close crosses below upper band
        """
        try:
    ...
    def execute_trade(self, symbol: str, signal: str) -> bool:
        try:
            ...
            # timeframe for ATR depends on mode
            ...
            if signal == 'BUY':
                ...
                if volume > 0:
                    ...
                    if success:
                        logger.info(
                            f"BUY {symbol}: Vol={volume}, Price={entry_price:.5f}, SL={stop_loss:.5f}, TP={take_profit:.5f}"
                        )
                ...
                if volume > 0:
                    ...
                    if success:
                        logger.info(
                            f"SELL {symbol}: Vol={volume}, Price={entry_price:.5f}, SL={stop_loss:.5f}, TP={take_profit:.5f}"
                        )
                        self._mark_trade_time(symbol)
                    return success
...
if __name__ == "__main__":
    # Create the robot with scalping enabled on M5 (adjust to TIMEFRAME_M1 for 1-minute)
    robot = MT5ForexRobot(
        risk_per_trade=0.01,           # Tighter risk for scalping
        magic_number=11123,            # Unique robot ID
        max_drawdown_percent=25.0,     # Stop trading if 25% drawdown
        drawdown_period_hours=24,      # Monitor drawdown over 24 hours
        enable_scalping=True,
        scalping_timeframe=mt5.TIMEFRAME_M5,
    ...
    try:
        robot.run_trading_bot(symbols, run_duration_minutes=30)
